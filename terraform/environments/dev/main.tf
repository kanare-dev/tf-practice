...
locals {
  ...
  # 例: "dev.note-app.kanare.dev" → "dev.note-app"
...
module "s3_static_web" {
  ...
  tags = {
    Name        = "${var.env}-tfpractice-s3-web"
    Environment = var.env
    System      = "tfpractice"
    ManagedBy   = "terraform"
  }
...
resource "aws_cloudfront_distribution" "note_app" {
  ...
  tags = {
    Name        = "${var.env}-tfpractice-cloudfront"
    Environment = var.env
    System      = "tfpractice"
    ManagedBy   = "terraform"
  }
...
module "lambda_api_handler" {
  ...
  tags = {
    Name        = "${var.env}-tfpractice-lambda-api"
    Environment = var.env
    System      = "tfpractice"
    ManagedBy   = "terraform"
  }
...
module "api_gateway" {
  ...
  api_name             = "note-api-gateway-${var.env}"
...
resource "cloudflare_record" "acm_validation_api" {
  ...
  type    = "CNAME"
  proxied = false # DNS only
  comment = "ACM certificate validation for ${var.api_domain_name}"
}
...
resource "cloudflare_record" "note_app" {
  ...
  type    = "CNAME"
  proxied = false # DNS only
  comment = "CloudFront distribution for ${var.domain_name} static site"
}
...
resource "cloudflare_record" "api_note_app" {
  ...
  type    = "CNAME"
  proxied = false # DNS only
  comment = "API Gateway custom domain for ${var.api_domain_name}"
}
...
module "cognito" {
  ...
  user_pool_name        = "note-app-user-pool-${var.env}"
  ...
  tags = {
    Name        = "${var.env}-tfpractice-cognito"
    Environment = var.env
    System      = "tfpractice"
    ManagedBy   = "terraform"
  }
...
module "notes_table" {
  ...
  tags = {
    Name        = "${var.env}-tfpractice-dynamodb-notes"
    Environment = var.env
    System      = "tfpractice"
    ManagedBy   = "terraform"
  }
